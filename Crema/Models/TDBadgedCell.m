...
- (void) layoutSubviews
{
	...
	if(self.badgeString)
	{
...
#endif
		CGRect badgeframe = CGRectMake(self.badgeLeftOffset,
        ...
        for (UILabel *label in self.resizeableLabels)
        {
            ...
            {
                CGFloat textLabelWidth = label.frame.size.width - badgeframe.size.width - self.badgeLeftOffset;
                label.frame = CGRectMake(label.frame.origin.x + badgeframe.size.width + self.badgeLeftOffset, label.frame.origin.y, textLabelWidth, label.frame.size.height);
            }
        }
